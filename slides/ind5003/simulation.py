...
def c1(env, cust_num, counter, service, cust_dict, verbose):
    ...
    #print('{0:.3f} {1}: Here I am'.format(arrive, str(cust_num)))
...
def c2(env, cust_num, counter, cust_dict, verbose):
    ...
    with counter.request() as req:
        ...
        if req in results:
            ...
            if verbose:
                print('SERV: Now the time is {0:.3f}, {1} waited {2:.3f}'.format(env.now, str(cust_num), wait))
            service_time = rng.exponential(3)
            cust_dict[cust_num][2] = service_time
            yield env.timeout(service_time)
            if verbose:
                print('DEP: {} leaving, at {:.3f}'.format(str(cust_num), env.now))
        else:
            cust_dict[cust_num][1] = wait
            cust_dict[cust_num][3] = 1 # means did renege
            if verbose:
                print('REN: {} leaving, at {:.3f}'.format(str(cust_num), env.now))
