...
Python is an open-source software and thus it is free to use and extend.
...
![Jupyter Lab interface](figs/jupyter_lab2.png){#fig-jupyter fig-alt="Jupyter Lab" fig-align="center"}

In @fig-jupyter, the red box labelled 1 is a markdown cell. It can contain
formatted text. The cells in the box labelled 2 are code cells. To run the codes
from our notes, you can copy and paste the codes into a new cell, and then
execute them with Ctrl-Enter.
...
[this website](https://jupyterlab.readthedocs.io/en/stable/user/interface.html). 

## Basic Data Structures in Python

The main data structures in native[^02-python-1] Python are
...
By the way, this is what mean by (im)mutable. The following is OK, because
`x_list` is a list, and hence mutable. In other words, elements inside it can be
overwritten.
...
But the following code will return an error, because `x_tuple` is a tuple, and hence 
immutable.
...
Try out the following two applications of the slice operator:
...
To obtain the length of a list-like object, we can use the inbuilt `len()`
function.
...
This slice syntax is used in the additional packages we use as well, so it is
good to know about it. @fig-py-slice displays a pictorial representation of how
positive and negative indexes work together.

![Positive and negative indices with the slice operator](figs/python_slice.png){#fig-py-slice fig-alt="Python slice" fig-align="center" width="55%"}
...
**immutable** sequence of characters. In this section, we shall cover some 
...
The output from the above informs us that `Carmen` is located at position 22 in
`test_str`.
...
time. See @sec-01-ref for a good introduction.

Remember that a string is immutable, just like a tuple, so this assignment will
*not* work:
...
The method to convert a string to lower case is:
...
simply load the module. We would then have access to the updated functions. We
access objects within the module using the "dot" notation. There are several
...
We shall practice more with writing classes in Python in chapter 7 (@sec-07-top)
when we use the Mesa package.
...
and then fill it in later. This is preferable to growing an array by appending
to it.
...
Similarly, there is also an `np.ones()` function. Instead of specifying the
dimensions of an array ourselves, we can create arrays of zeros or ones in the
shape of other existing arrays. The following code creates an array of ones, of
the same shape as `arr_real`.
...
An alternative to using integers in the selection index is to use Boolean
indexing. This means that we use an array of `True` and `False` entries to
...
The next few commands generate $Unif(0,1)$ random variates and store them in 
rectangular arrays. We shall see more about these particular functions in the 
chapter on Simulation (@sec-07-top), but for now, take note that the number 
`1361` is known as a seed; it ensures the reproducibility of random numbers 
across sessions.
...
Similarly, the `*` operator performs element-wise multiplication.

```{python}
a * b
```

For matrix multiplication, we have to transpose `b`.
...
Numpy and Pandas have several facilities for applying functions across array 
axes. It is important to be aware of how they work. For instance, this takes the
mean across the 0th (first) axis. The resulting array has shape `(3,4)`.
...
For instance, if we wanted to identify the row with the largest mean, we use
`argmax()` on the resulting array.
...
Be careful when you combine the slice operator with label-based indexing. Unlike
vanilla Python, Pandas includes **both** end-points!
...
::: {#exm-pd-df-1 style="background-color: #D5D1D164; padding: 20px"}

### Creating Pandas DataFrame from Series
\index{Liga!DataFrame}
...
GDP per capita, family, freedom etc) which is included in the file as well.
...
In dataframes, row selection can *technically* be done with integers with the
slice operator. In practice, this is not used, because we typically wish to
...
inclusive when we use `.iloc`. Consider the following output, using `.loc`.
...
In order to obtain the same output with `.iloc`, we have to use this instead:
...
::: {#exm-pd-happ-2 style="background-color: #D5D1D164; padding: 20px"}

### Happiest countries
\index{Happiness!Happiest countries}

Suppose we retrieve the happiest countries; those with a score more than 6.95.

::: {.callout-note}
Can you surmise why we use this value? 
:::
...
1.  Compute the mean of the `arr_real` array, by the second axis. You should obtain:
```{python}
#| echo: false
arr_real.mean(axis=1)
```
2.  Use Boolean indexing on the `laliga` DataFrame to retrieve the rows where 
    Valencia had more than 10 defeats.
3.  If a win is 3 points, a draw is 1 point and a loss is 0, compute the number 
    of points the team obtained for each row in the `laliga` DataFrame. Store the
    results in a new column named `pts`.
4.  What does the following code do? How different is it from a conventional loop
    in Python?
```{python}
#| eval: false
olist = [dict1[x] for x in dict1.keys()]
```
5.  Look up the `str` methods in Python and use the appropriate one to convert 
    `test_str` into this:
```{python}
#| echo: false
test_str.title()
