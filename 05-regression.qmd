...
@fig-taiwan-scatter shows a scatter plot of the points in the dataset.
...
Let us first explore the dataset with Python. Take a look at the plots in
@fig-taiwan-eda. How do they aid in your understanding of the dataset?
...
As a first model, we fit price ($Y$) against house age ($X_1$). From the plot in
@fig-taiwan-eda earlier, we already suspect this may not be ideal, but let us
...
output includes the 95% confidence intervals for $\beta_0$ (40.1, 44.8) and
$\beta_1$ (-0.4, -0.1). The $R^2$ is 0.044, which means that means that only
4.4% of the variation in $Y$ is explained by $X$. This is extremely poor, even
though the $p$-value for the $F$-test is very small (0.000016).
...
::: {.callout-warning}
Note that this interpretation has to be taken very cautiously, especially when
there are other explanatory variables in the model.
:::
...
is how we can do so for the model that we have just fit. The accompanying plot in 
@fig-taiwan-mod1-ci shows the predicted mean for various house age, along with the 
confidence intervals for these means. 
...
different from 0. Moreover, the adjusted $R^2$ is now a more respectable 0.49. This 
is a huge improvement from @exm-taiwan-reg-2.
:::
...
Visually, we can inspect the estimated lines in @fig-taiwan-mod3-broken-line.
...
X_4 = 
\begin{cases}
0, & \text{if there were 4 nearby stores or less} \\
1, & \text{otherwise}
\end{cases}
$$
...
Let us inspect the histogram and qq-plot for the residuals from the model in
@exm-taiwan-reg-4.
...
From @fig-taiwan-norm-check, it appears that the distribution is slightly
skewed, and there is one noticeable outlier. The qq-plot does suggest fatter 
tails than the Normal on the right, even if we were to set aside the outlier.
:::
...
We typically inspect the plots for the following patterns (see @fig-example-resid-patterns):

![Examples of residual plot patterns](figs/regression_residuals.png){#fig-example-resid-patterns width=80%}
...
### Residual Plots for `lm_age_mrt_2`
\index{Taiwan Data!Residual scatterplots}

Let us extract and create the residual plots for the second model that we had fit, 
earlier in @exm-taiwan-reg-5.
...
something we can try to fix, using a transformation of the x-variable (see 
@sec-regression-transformation).
:::
...
performs a similar iteration to assess the influence on the fitted values.
...
adjustment for those points. A third approach is to use a *robust linear model*. 
This is a model that automatically reduces the influence of aberrant points.
This is a good topic to know about - do read up on it if you are keen!
...
In this example, we inspect the influential points in the model that we fitted in 
@exm-taiwan-reg-5.
...
We can see from the output that the point with index 270 has a large Cook's distance,
and also has quite a large influence on the $\beta$-estimate for `dist_MRT`. This is 
...
As we observed in the residual plots in @fig-taiwan-resid-checks, the
distance-to-MRT variable displays a slight curvature. We can fix this by taking
a log-transformation of the variable before fitting the model. Below, we include
the code to perform this fitting.
...
This model turns out to have one of the highest adjusted $R^2$ that we have encountered 
in this chapter. However, that should not be the criteria for deciding on the model.
As analysts, we should fully understand the model's strengths and weaknesses before 
using it. Toward that end, take a look at the questions in the @sec-reg-exercises.
:::
...
Take some to investigate the following issues pertaining to the model fitted in 
@exm-taiwan-reg-11.
