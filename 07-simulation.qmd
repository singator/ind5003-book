...
We can visualise a pmf using a bar-chart. @fig-pmf-biased-coin contains the
chart for the above biased coin.
...
@fig-pmf-ten-tosses displays the pmf for a random variable representing the
total number of Heads after 10 tosses of that same coin. Suppose we call that
...
The diagram in @fig-agent-flow depicts the flow of events for each agent. It is
useful to sketch such a flowchart when implementing agent-based models, since
it allows for easier debugging and information sharing with colleagues.

![Agent trajectory](figs/money_agent_flowchart.png){#fig-agent-flow fig-align="center" width=50%}

The next class defines the model itself. The initialisation of a model
typically consists of populating it with various agents. At every time time
step, the agents are shuffled, and then each of their `step` methods are
executed in that order. The order changes at every time step. It is necessary
to avoid running all agents' steps at the same time to avoid clashes.
...
We initialise the random number generator using a seed (not an `rng` object)
and re-run the model. We can then retrieve the zero-wealth proprortion at
...
We can see from @fig-prop-zero-mult-iterations that the proportion of agents
with zero income is considerably higher than the 0.0 that the population began
with.
...
![Moore neighbourhood](figs/Moore_neighborhood_with_cardinal_directions.png){#fig-moore-nbhd fig-align=center width=30%}
...
with 100 agents in a 10x10 grid (see @fig-gini-coef-2).
...
The next example shows how we can use `mesa` to generate an interactive visualisation
of how the model evolves step-by-step.

::: {#exm-boltzmann-7 style="background-color: #D5D1D164; padding: 20px"}

### Boltzmann model visualisation
\index{Boltzmann Model!v3 interactive}
...
You should observe an application just like the one in #fig-mesa-vis.

![Mesa visualisation](figs/07-simulation-mesa-vis.png){#fig-mesa-vis fig-align=center width=50%}
...
1.  Re-run the codes in @exm-power-1 to recommend an appropriate sample size if
    the investigator wishes to detect a difference in means of 1, with power of
    at least 0.9, when testing at $\alpha=5\%$, but if the standard deviation
    is known to be 2.5?
2.  Report a 80% confidence interval for the power for the recommended sample
    size in question 1.
3.  Similar to the class definition for a circle, define a Python class for an 
    ellipse. The class should include a method for computing the area of an
    ellipse.
4.  Modify the Boltzmann model in @exm-boltzmann-5 to compute a 95% confidence
    interval for the Gini coefficient after 100 steps.
5.  Implement the code for the Insurance claims (see @exm-insurance-1). Return
    a 90% confidence interval for the probability that claims exceed 12000.
