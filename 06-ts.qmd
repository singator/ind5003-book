...
In @fig-one-fam-houses, we observe a strong seasonality within each year. There
is some indication of cyclic behaviour every 6 -- 10 years. There is no
apparent monotonic trend over this period. With pandas objects, we can resample
the series. This allows us to compute summaries over time windows that could be
of business importance. For instance, we might be interested in a breakdown by
...
@fig-aus-elec-time shows a very clear increasing trend. There is strong
seasonality, and there is no evidence of cyclic behaviour (see
@fig-season-qauselec).
...
@fig-lag-sales displays a matrix of lag plots for the housing sales data (see
@fig-one-fam-houses and @fig-season-sales for reference).
...
@fig-add-decomp-hsales contains the naive additive decomposition of the housing
sales data.
...
@fig-mult-decomp-qau the *multiplicative decomposition* for the Australian electric data.
...
withhold the most recent 2 years of data and forecast those. Take a look at 
@fig-benchmark-hsales, that depicts the forecasts along with the true values.
...
Let us assess the forecasts made in @exm-housing-4, using RMSE and MAE.
...
Consider the following time series of the Dow Jones index, shown in
@fig-dow-ts.
...
However, the following differenced version of the same series is stationary.
Take a look at @fig-dow-diff-ts to see the constant mean and variance depicted.
...
Let us proceed to make a plot of the predictions (see @fig-arima-fc).
...
![ETS additive models](figs/ets_add.png){#fig-ets-add-models width=80%}

![ETS multiplictive models](figs/ets_mult.png){#fig-ets-mult-models width=80%}
...
to Singapore at the monthly level. @fig-sg-tourism-ts contains a time series
plot of arrivals from South East Asian Countries:
...
Finally, we visualise the forecasts from this theta decomposition model in
@fig-theta-decomp-fc. We only plot the final 360 observations for the
visualisation.
...
pre-chosen ARIMA(3,1,2) model to the seasonally adjusted series. The output
forecasts can be seen in @fig-decomp-fc.
...
@fig-emp-ts-example contains a time series plot of one of the time series in
the dataset.
...
a 148 x 148 distance matrix. For more details on hierarchical clustering, refer
to @sec-07-clustering. The resulting dendrogram can be seen in
@fig-emp-dendrogram.
...
next to one another (see @fig-heatmap-correl).
...
1.  Create a lag plot, with 8 lags, of the Australian electricity data in
    `qauselec.csv`.
2.  Extract the residuals from applying the seasonal naive model to the
    Australian electricity data, and compare the residual plots to the 
    ones in @fig-arima-diag.
3.  Apply the auto ARIMA procedure to the housing sales data, and inspect the 
    residuals.
4.  Apply the ETS (M,A,A) model to the Australian electricity data. Compare the 
    RMSE and residuals to the ETS model fitted in @exm-aus-elec-5.
5.  Apply the auto ARIMA procedure directly to the tourism data, and compare it
    to the theta model and the seasonal decomposition one in @exm-tourism-2.
