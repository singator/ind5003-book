...
we look for an "elbow" shape to inform us of how many to keep. Judging from 
@fig-wine-scree, we would probably want to retain 4 or 5 principal components.
...
that end with an underscore. Above, we can see that the first principal component
explains 27.5% of the total variation. The first two components explain 50.2% of
...
coefficients as well as possible (see @tbl-wine-loading2).
...
In social sciences especially, it is a convention to drop loadings that are
smaller than 0.3 in absolute value and then to interpret the remaining
coefficients as well as possible (see @tbl-wine-loading).
...
Judging from the plots in @fig-pc-rating, red wines tend to be lower on PC1. Can
we tie this back to the columns in the data to understand the difference between
white and red wines more?
...
important to choose carefully based on the subject matter.
@fig-sklearn-clustering, from `sklearn` website, demonstrates the impact of the 
choice of linkage on the clusters in toy datasets.

![sklearn documentation](figs/sklearn_linkage_comparison_001.png){#fig-sklearn-clustering  fig-align="center" width=70%}
...
As we can see the number of clusters changed from 3 to 2 and then to 1. @fig-toy-dendrogram
shows a visualisation of the hierarchy in the toy dataset.
...
As we can see from @fig-wine-scatter-pc, the groupings closely mirror the type
...
However, notice from @tbl-wine-clusters that there are a number of white wines
grouped as 0 (with most of the other reds). It would be interesting to study
what qualities of these wines led to them being grouped with the reds.
...
We can see from @tbl-taiwan-outliers2 that the outliers are very different from
the remaining points: the distance to MRT, X-coordinates, and price are all very
different. Having identified these points, our job as analysts is to interpret
the differences.
...
We can see from @tbl-taiwan-outliers that the outliers are very different from
the remaining points: the distance to MRT, X-coordinates, and price are all very
different. Having identified these points, our job as analysts is to interpret
the differences.
...
In this pdf version in @fig-mds-disease, we display a static image. However, the
online version contains an interactive plot.
...
models provided and maintained by [Hugging Face](https://huggingface.co/). The first 
line in the code below retrieves a token from the operating system. This token
authenticates with Hugging Face before downloading begins. The following [web-page](https://www.cs.virginia.edu/~rmw7my/Courses/AgenticAISpring2026/COMPLETE_HF_AUTH_GUIDE.html) has more detailed instructions on generating and adding a token to your computer.
...
Please refer to the online version of the text for an interactive plot (@fig-tsne-tweets).
...
1.  Apply t-SNE to visualise the wine quality data. Do you observe similarities 
    in the points that appear close to each other? For instance, do red wines tend 
    to be close to each other?
2.  Refer to the disease symptoms dataset. What symptoms are common to Hepatitis 
    A, B, C, D and E?
3.  Apply the PCA technique to the Taiwan dataset. How many components will you 
    need to explain 80% of the variation in the data?
4.  Consider the groups identified by hierarchical clustering in Example 3.4. Which
    variables in the original wine quality data are most associated with this grouping?
    You will have to use the methods from Chapter 2, and you will have to convert the 
    numeric columns into categories (e.g. High, Low, etc.).